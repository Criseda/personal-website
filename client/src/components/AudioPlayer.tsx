...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ jobId, format = 'mp3' }) => {
    ...
    return (
      <div className="bg-white/40 dark:bg-black/40 backdrop-blur border border-purple-500/20 dark:border-purple-500/30 rounded-lg p-6">
        <div className="flex items-center justify-center gap-3">
          ...
          <p className="text-zinc-700 dark:text-white text-sm">Loading audio...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-white/40 dark:bg-black/40 backdrop-blur border rounded-lg p-6 space-y-4 transition-all duration-300 ${isPlaying
      ? 'border-cyan-500/50 bg-gradient-to-br from-white/50 dark:from-black/50 via-cyan-50 dark:via-cyan-900/10 to-white/50 dark:to-black/50 shadow-lg shadow-cyan-500/10 dark:shadow-cyan-500/20'
      ...
      <div className={`flex items-end justify-center gap-1 h-12 mb-2 transition-opacity duration-300 ${isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}>
      ...
      {/* Now Playing */}
      <div>
        <p className="text-xs text-zinc-500 dark:text-gray-400 mb-1">Now Playing</p>
        <p className={`font-semibold text-sm transition-colors ${isPlaying ? 'text-cyan-600 dark:text-cyan-300' : 'text-zinc-900 dark:text-white'
          }`}>
      ...
      {/* Last generation log */}
      {logs.length > 0 && (
        <div className="bg-zinc-100/50 dark:bg-black/50 border border-purple-500/10 dark:border-purple-500/20 rounded p-2">
          <p className="text-xs text-emerald-900 dark:text-green-400 font-mono">
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <div className="relative w-fit">
          ...
          <button
            onClick={togglePlay}
            className={`relative w-14 h-14 rounded-full flex items-center justify-center text-white text-xl transition-all hover:scale-110 active:scale-95 font-semibold ${isPlaying
        ...
        <div className="flex-1">
          ...
          <input
            ...
            value={currentTime}
            onChange={handleSeek}
            className="w-full h-2 bg-zinc-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-cyan-500 transition-all"
            style={{
              background: `linear-gradient(to right, ${isPlaying ? '#06b6d4' : '#a855f7'
                } 0%, ${isPlaying ? '#06b6d4' : '#a855f7'} ${(currentTime / duration) * 100
                }%, ${isPlaying && document.documentElement.classList.contains('dark') ? '#374151' : '#e5e7eb'} ${(currentTime / duration) * 100}%, ${isPlaying && document.documentElement.classList.contains('dark') ? '#374151' : '#e5e7eb'} 100%)`,
            }}
          />
      ...
      {/* Additional Info */}
      <div className={`text-xs text-center pt-2 border-t transition-colors ${isPlaying
        ? 'border-cyan-500/30 text-cyan-600 dark:text-cyan-400'
        : 'border-purple-500/10 dark:border-purple-500/20 text-zinc-500 dark:text-gray-400'
        }`}>
